...
const DateSelect = ({ dateTime = {}, id }) => {
  ...
  const onBookHandler = () => {
    ...
    window.scrollTo(0, 0); // added explicit window
  };

  if (!dateTime || Object.keys(dateTime).length === 0) {
    return (
      <div className="pt-30 px-6 md:px-16 lg:px-36 mt-20 text-white">
        No dates available.
      </div>
    );
  }
      ...
      <div className="relative p-8 rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md shadow-xl overflow-hidden">
        <div className="flex flex-col md:flex-row items-center justify-between gap-10 relative z-10">
                ...
                {Object.keys(dateTime).map((date) => (
                  <button
                    ...
                    className={`flex flex-col items-center justify-center h-16 w-16 rounded-xl cursor-pointer relative transition-all duration-300
                      ${
                        selected === date
                          ? 'scale-105 border-2 border-[#E63946] bg-[#E63946] text-white shadow-[0_0_8px_rgba(230,57,70,0.8)]'
                          : 'scale-100 border border-red-400/70 bg-white/10 text-white hover:bg-red-700/40'
                      }
                    `}
                    style={{
          ...
          <button
            ...
            className="bg-red-600 text-white px-10 py-3 mt-6 md:mt-0 rounded-full hover:bg-red-700 hover:scale-105 transition-all duration-300 shadow-md"
            type="button"
          >
            Book Now
          </button>
